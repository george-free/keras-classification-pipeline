...
class DeeplabV3(ClassificationModel):
    def build_network(self,  
        ...
        if self._train_from_scratch:
            print("---------------- build deeplabv3 network from scratch --------------")
        ...
        x = Conv2D(self._num_class, (1, 1), padding='same', name=last_layer_name)(x)
        ...
        # classification maps
        # x = Flatten()(x)
        # x = Dense(1024, name='fc_feature_embedding')(x)
        # x = Dense(self._num_class, name='fc_classes')(x)
        x = AveragePooling2D((16, 16), name='avg_pooling')(x)
        x = Flatten()(x)
        ...
        if not self._train_from_scratch:
            print("---------------- load deeplabv3 network for train --------------")
